...
struct MarkdownCommentTests {
    ...
    @Test func reviewScriptPositionsCommentCardsByAnchor() {
        ...
        #expect(source.contains("behavior: 'smooth'"))
        #expect(source.contains("activeItem"))
        #expect(source.contains("nex-comment-card-suppressed"))
        #expect(source.contains("var suppressedHeight = 52"))
        #expect(source.contains("activeAboveCursor"))
        #expect(source.contains("Math.min(height, suppressedHeight)"))
